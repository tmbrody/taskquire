...
const UpdateTaskPage: React.FC<UpdateTaskPageProps> = () => {
    ...
    const { orgName, projectName, teamName, taskName, subtaskName, subtaskDescription } = router.query;

    const [name, setName] = useState(subtaskName || '');
    const [description, setDescription] = useState(subtaskDescription || '');

    const handleUpdateSubtask = async (event: React.FormEvent) => {
        ...
        router.push(`http://localhost:3000/orgs/${orgName}/projects/${projectName}/teams/${teamName}/tasks/${taskName}/subtasks`)
    }
